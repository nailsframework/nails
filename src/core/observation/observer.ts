        ...
        const handler = {
            get(targe: any, property: any) {
                ...
                if (property === '__getTarget') {
                    ...
                    const parentPath = _getPath(target, '__getParent', undefined);
            ...
            set(target: any, property: any, value: any, receiver: any) {
                ...
                if (targetProp !== value || originalChange === false || (property === 'length' && target instanceof Array && target.length !== value)) {
                    ...
                    // so in order to accurately report the correct previousValue for the .length, we have to use a helper property.
                    if (property === 'length' && target instanceof Array && target.length !== value) {
                        changes[changes.length - 1].previousValue = target.length;
                        target.length = value;
                    }
    ...
    return {
		/*	Method:
			...
			Returns:
				An ES6 Proxy object.
		*/
        create(target: any, domDelay: any, observer: any) {
		...
		/*	Method: observe
			...
			Returns:
				Nothing.
		*/
        observe(proxy: any, observer: any) {
